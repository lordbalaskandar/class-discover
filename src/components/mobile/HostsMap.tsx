...
declare global {
  interface Window {
    google?: any;
    __dryvonInitMap?: () => void;
    __dryvonMapReady?: boolean;
  }
}

const SCRIPT_ID = "dryvon-gmaps-js";

function loadGoogleMaps(): Promise<void> {
  if (typeof window === "undefined") return Promise.resolve();
  if (window.__dryvonMapReady && window.google?.maps) return Promise.resolve();
  return new Promise((resolve, reject) => {
    if (document.getElementById(SCRIPT_ID)) {
      const check = () =>
        window.__dryvonMapReady ? resolve() : setTimeout(check, 80);
      check();
      return;
    }
    window.__dryvonInitMap = () => {
      window.__dryvonMapReady = true;
      resolve();
    };
    ...
    s.async = true;
    s.defer = true;
    s.src = `https://maps.googleapis.com/maps/api/js?key=${key}&loading=async&callback=__dryvonInitMap${channel ? `&channel=${channel}` : ""}`;
